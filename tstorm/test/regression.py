...
leading zeroes matters.'''
      ...
      lcgls = ls.LcgLs(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], self.dfn)
      self.lfn.put_cmd(lcgls.get_command())
      ll = lcgls.get_output()
      self.assert_(ll['status'] == 'FAILURE')

      lcgcp = cp.LcgCp(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], self.ifn, self.dfn, self.bifn)
      self.lfn.put_cmd(lcgcp.get_command())
      self.cp_result = lcgcp.get_output()
      ...
      stormrm = rm.StoRMRm(self.tsets['general']['endpoint'], self.tsets['https']['voms'], self.dfn)
      self.lfn.put_cmd(stormrm.get_command())
      self.rm_result = stormrm.get_output()
      self.assert_(self.rm_result['status'] == 'PASS')
      if '/' in self.dfn:
        a=os.path.dirname(self.dfn)
        stormrmdir = rmdir.StoRMRmdir(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], a)
        self.lfn.put_cmd(stormrmdir.get_command())
        self.rmdir_result = stormrmdir.get_output()
        for x in self.rmdir_result['status']:
          self.assert_(x == 'PASS')

      rmlf = removefile.RmLf(self.ifn, self.bifn)
 ...
 BDII due to a bug in the update of the free space after the the srmRm operation'''
      ...
      stormgst = space.StoRMGst(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], self.tsets['general']['spacetoken'])
      self.lfn.put_cmd(stormgst.get_command())
      self.st_result = stormgst.get_output()
      self.assert_(self.st_result['status'] == 'PASS')

      stormgsm1 = space.StoRMGsm(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], self.st_result['arrayOfSpaceTokens'])
      self.lfn.put_cmd(stormgsm1.get_command())
      self.sm1_result = stormgsm1.get_output()
      ...
      lls = ls.Ls(self.ifn)
      self.lfn.put_cmd(lls.get_command())
      self.lls_result = lls.get_output()
      self.assert_(self.lls_result['status'] == 'PASS')

      stormgsm2 = space.StoRMGsm(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], self.st_result['arrayOfSpaceTokens'])
      self.lfn.put_cmd(stormgsm2.get_command())
      self.sm2_result = stormgsm2.get_output()
...
operation.'''
      ...
      lcgls = ls.LcgLs(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], self.dfn)
      self.lfn.put_cmd(lcgls.get_command())
      self.ls_result = lcgls.get_output()
...
if the provided protocols are supported.'''
      ...
      stormptp = cp.StoRMPtp(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], self.dfn, 'unsupported')
      self.lfn.put_cmd(stormptp.get_command())
      self.ptp_result = stormptp.get_output()
...
desired transfered protocols.'''
      ...
      stormptp = cp.StoRMPtp(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], self.dfn, self.prt + ',unsupported')
      self.lfn.put_cmd(stormptp.get_command())
      self.ptp_result = stormptp.get_output()
      self.assert_(self.ptp_result['status'] == 'PASS')

      fsgrep = findstrings.Grep().get_output()
      self.lfn.put_cmd(fsgrep.get_command())
      self.fs_result = fsgrep.get_output()
      self.assert_(self.fs_result['status'] == 'FAILURE')

      stormar = abort.StoRMAr(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], self.ptp_result['requestToken'
      self.lfn.put_cmd(stormar.get_command())
      self.ar_result = stormar.get_output()
...
providing string parameters containing non ASCII characters.'''
      ...
      stormls = ls.StoRMLs(self.tsets['general']['endpoint'], self.tsets['general']['accesspoint'], self.dfn + 'tèstèèà').ge
      self.lfn.put_cmd(stormls.get_command())
      self.ls_result = stormls.get_output()
      self.assert_(self.ls_result['status'] == 'FAILURE')

      stormping = ping.StoRMPing(self.tsets['general']['endpoint'])
      self.lfn.put_cmd(stormping.get_command())
      self.ping_result = stormping.get_output()
