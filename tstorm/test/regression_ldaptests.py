...
def glue_service_ts(conf, uid, lfn):
    ...
    s.addTest(lq.LdapTest('test_glue_service', conf, uid, lfn))

    return s

def glue_storage_share_capacity_ts(conf, uid, lfn):
    s = unittest.TestSuite()
    s.addTest(lq.LdapTest('test_glue_storage_share_capacity', conf, uid, lfn))
