...
def ts_prepare_to_put_wrong_space_token(conf, ifn, dfn, bifn, uid, lfn):
    ...
    s.addTest(re.RegressionTest('test_prepare_to_put_wrong_space_token',conf, ifn, dfn, bifn, uid, lfn))
    return s
...
def ts_prepare_to_put_expired_space_token(conf, ifn, dfn, bifn, uid, lfn):
    s = unittest.TestSuite()
    s.addTest(re.RegressionTest('test_prepare_to_put_expired_space_token',conf, ifn, dfn, bifn, uid, lfn))
    return s

def ts_get_space_metadata_on_valid_space_token(conf, ifn, dfn, bifn, uid, lfn):
    s = unittest.TestSuite()
    s.addTest(re.RegressionTest('test_get_space_metadata_on_valid_space_token',conf, ifn, dfn, bifn, uid, lfn))
    return s
...
def ts_storm_gridhttps_authorization_denied_on_no_sub_string(conf, ifn, dfn, bifn, uid, lfn):
    s = unittest.TestSuite()
    s.addTest(re.RegressionTest('test_storm_gridhttps_authorization_denied_on_no_sub_string', conf, ifn, dfn, bifn, uid, lfn))
    return s
