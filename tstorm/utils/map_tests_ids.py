...
class MapTestsIds:
    ...
    def __get_all_tests_(self):
        '''Returns a dictionary containing all categories tests'''

        all_tests = {}

        for key, value in self.categories_tests.items():
            all_tests[eval('self.' + x) = value
    ...
    def __get_tests_tuple(self):
        '''Returns a dictionary containing a map between test and id'''

        test_ids = {}

        for key, value in self.__get_all_tests_.items():
            for k, val in key.keys():
                if val[0] == '':
                    uuid = utils.get_uuid()
                    if len(uuid) > 6:
                        val[0] = uuid[:6]
                        ...
                        val[0] = uuid
                if 'ts_' in k:
                    test_ids[k] = (val[0], value[1], value[2], va[1], val[2], val[3], val[4])
                else:
                    test_ids[k] = (val[0], value[1], value[2], va[1], val[2], val[3])
    ...
    def __set_map_file_entry_(self, map_len, current_index, file_index, key, value):
        '''Set an entry in the map file'''
    ...
    def create_new_map_file(self):
        ...
        map_tests_ids = self.__get_tests_tuple()
        ...
        for x in map_tests_ids.keys():
            self.__set_map_file_entry_(len(map_tests_ids), a+1, df, x, map_tests_ids[x])
