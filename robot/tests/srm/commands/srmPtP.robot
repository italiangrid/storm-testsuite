...
srmPtP using a non existent space token
  ...
  check srmPtP failure with  ${outputPtP}  SRM_FAILURE
  ${outputSPtP}  srmSPtP  ${surl}  ${token}
  check srmSPtP failure with  ${outputSPtP}  SRM_FAILURE
  Should Contain  ${outputSPtP}  Unable to reserve space on storage area
  [Teardown]  Clear all credentials
