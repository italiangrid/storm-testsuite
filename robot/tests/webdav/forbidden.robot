...
WebDAV GET/HEAD VO file as anonymous user and anonymous http read disabled
  ...
  ${url}  Build URL  ${webdavEndpoint}  ${TEST_SA}  ${TEST_REMOTE_DIRNAME}/${TEST_FILENAME}
...
WebDAV MKCOL VO directory as anonymous
  ...
  [Setup]  Setup default SA
  Create empty working directory
  ${url}  Build URL  ${webdavEndpoint}  ${TEST_SA}  ${TEST_REMOTE_DIRNAME}/${TEST_REMOTE_DIRNAME}
...
WebDAV COPY VO file as anonymous
  ...
  [Setup]  Setup default SA
  Create working directory
  ${srcURL}  Build URL  ${webdavEndpoint}  ${TEST_SA}  ${TEST_REMOTE_DIRNAME}/${TEST_FILENAME}
  ${dstURL}  Build URL  ${webdavEndpoint}  ${TEST_SA}  ${TEST_REMOTE_DIRNAME}/${TEST_FILENAME}_2
...
WebDAV COPY VO file to another VO SA with the wrong proxy
  ...
  Should Contain  ${stdout}  Local copy across storage areas is not supported
  [Teardown]  Teardown default SA

WebDAV COPY VO file to another VO SA with the right proxy
  [Tags]  webdav  forbidden  copy
  [Setup]  Setup SA and VO  ${webdavSecureEndpoint}  ${SA.9}  ${DEFAULT_USER}  ${VO.1}
  Create working directory
  ${srcURL}  Build URL  ${TEST_ENDPOINT}  ${SA.9}  ${TEST_REMOTE_DIRNAME}/${TEST_FILENAME}
...
WebDAV COPY VO file into the same storage area with the wrong proxy
